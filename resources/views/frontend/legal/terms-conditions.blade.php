...
@section('title', $termsConditions->title . ' - Genesis Accommodation')

@section('body')
    <div class="mx-auto max-w-screen-2xl">

        {{-- @dd($termsConditions->subtitle) --}}
        <!-- Page Header -->
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 py-12">
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="text-center">
                    ...
                    <!-- Test message to verify page is loading -->
                    <div class="mt-2 text-sm text-blue-600">
                        Page loaded successfully - Testing subtitle display
                    </div>


                    <p class="mt-3 max-w-2xl mx-auto text-xl text-gray-600 sm:mt-4">
                        {{ $termsConditions->subtitle ?? 'No subtitle available' }}
                    </p>
                    @if ($termsConditions->last_updated)
                        <div
                            class="mt-4 inline-flex items-center px-4 py-2 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                            Last updated: {{ $termsConditions->last_updated->format('F j, Y') }}
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class=" px-4 sm:px-6 lg:px-8 py-12">
            ...
            {{-- @if (str_contains($termsConditions->content, '<h2>'))
            <div class="mb-8 p-4 bg-gray-50 rounded-lg">
                <h3 class="text-lg font-semibold text-gray-900 mb-3">Quick Navigation</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    @php
                        preg_match_all('/<h2[^>]*>(.*?)<\/h2>/', $termsConditions->content, $matches);
                        $sections = $matches[1] ?? [];
                    @endphp
            ...
            <div class="mt-8 p-6 bg-blue-50 rounded-lg border border-blue-200">
                <div class="text-center">
                    <h3 class="text-lg font-semibold text-blue-900 mb-2">Questions About These Terms?</h3>
