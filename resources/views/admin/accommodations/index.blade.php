            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-3">
                    <div class="flex items-center space-x-3">
                        ...
                        <form method="POST" action="{{ route('admin.accommodations.toggle-display-all') }}" class="inline">
                            @csrf
                            @method('PUT')
                                    ...
                                    <td class="px-6 py-4 ">
                                        @if ($accommodation->featured_image)
                                            <img src="{{ asset('uploads/' . $accommodation->featured_image) }}"
                                                alt="{{ $accommodation->name }}" class="h-12 w-12 object-cover rounded-lg">
                                        @else
                                    ...
                                    <td class="px-6 py-4 ">
                                        <div>
                                            ...
                                            @if ($accommodation->city)
                                                <div class="text-xs text-gray-400">{{ $accommodation->city }},
                                                    {{ $accommodation->state }}</div>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-6 py-4  text-sm text-gray-900">
                                        @if ($accommodation->roomType)
                                            <div class="text-sm font-medium text-gray-900">
                                                {{ $accommodation->roomType->name }}</div>
                                            <div class="text-xs text-gray-500">
                                                ${{ $accommodation->roomType->price_per_night }}/night</div>
                                        @else
